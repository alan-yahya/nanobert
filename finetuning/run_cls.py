...
import evaluate
import numpy as np
from datasets import Value, load_dataset, load_metric
...
def main():
    ...
    def compute_metrics(eval_pred):
        metric0 = load_metric("accuracy",trust_remote_code=True)
        metric1 = load_metric("precision",trust_remote_code=True)
        metric2 = load_metric("recall",trust_remote_code=True)
        metric2 = load_metric("f1",trust_remote_code=True)
        metric2 = load_metric("roc_auc",trust_remote_code=True)
        ...
        roc_auc = metric4.compute(predictions=predictions, references=labels)["roc_auc"]
        return {"precision": precision, "recall": recall, "f1": f1, "roc_auc": roc_auc, "accuracy": accuracy}
    '''
    '''
